...
export class Arduino {
    ...
    private static _ArduinoWaitingReadyMessages: string[] = [];
    private onData(data: Buffer): void{
        ...
        if(cmd == "1"){
            ...
            Arduino._isArduinoReady = true;
            setTimeout(() => {
                if(Arduino._ArduinoWaitingReadyMessages.length > 0){
                    for(let i =0; i<Arduino._ArduinoWaitingReadyMessages.length; i++){
                        Arduino.send(Arduino._ArduinoWaitingReadyMessages[i]);
                    }
                }
            }, 250);
    ...
    private send(data: string): void{
        if(Arduino._isArduinoReady){
            console.log(`Serial W: ${data}`);
            this._port.write(`${data}\n`);
        }else Arduino._ArduinoWaitingReadyMessages.push(data);
    }
